...
Releases are tagged `pk-vX.Y.Z` (latest: [`pk-v0.3.15`](https://github.com/pykeko/Moorhen-PyKeko/releases/tag/pk-v0.3.15), 2026-07-12). The **`PyKeko.dmg` asset lives on _this_ repo's releases page** — that's been the convention since v0.1 and matches the org profile README's download badge / install link. The PyKeko wrapper repo (`pykeko/PyKeko`) carries a matching tag at the same version with a mirror of the dmg as a fallback, but the canonical download lives here. Version source of truth: `~/PyKeko/package.json` (currently `0.3.15`); this fork's `baby-gru/package.json` carries upstream Moorhen's version (`1.0.0-alpha.1`) and shouldn't be edited as part of PyKeko's versioning. The PyKeko wrapper's `CLAUDE.md` carries the full per-version changelog.
