#!/usr/bin/env python3
"""Bulk-fetch _struct_conn + chem_comp data for all BTK + EGFR entries via
the RCSB Data GraphQL API.

We batch by 25 entries per query to stay within rate limits.
"""
import json
...
def run(label):
    with open(f'/tmp/btk-egfr-inventory/{label}_all_ids.json') as f:
        ids = json.load(f)['ids']
    print(f"Fetching {len(ids)} {label} entries...")
    out_path = f'/tmp/btk-egfr-inventory/{label}_raw.jsonl'
